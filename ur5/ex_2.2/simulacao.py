...
Kp = 3 * np.diag([300, 300, 300, 300, 300, 300])
Kd = 2*np.sqrt(Kp)
...
mj_data.qvel[:6] = referencia_vel(0)    
...
with mujoco.viewer.launch_passive(mj_model, mj_data) as viewer:
    ...
    while mj_data.time <= 8.0:
        ...
        log_vel.append(np.copy(mj_data.qvel[:6]))
...
velocidade_referencia = [referencia_vel(ti) for ti in intervalo]
...
for i in range(3):
    for j in range(2):
        junta = i + j*3
        axs2[i][j].plot(intervalo, [vel[junta] for vel in velocidade_referencia])
        axs2[i][j].plot(intervalo, [vel[junta] for vel in log_vel])
        axs2[i][j].set_title(f'Junta {junta + 1}')
        axs2[i][j].grid()

plt.savefig('vel_juntas.pdf')
